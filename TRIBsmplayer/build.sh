...
env PATH=/usr/versions/Qt-5/bin:$PATH gmake PREFIX=/usr CC=gcc
cd ..
...
sed -i 's:-pthread:-lnsl -lsocket:' webserver/Makefile
sed -i 's:Another OS:Tribblix:' src/smplayer.cpp
env PATH=/usr/versions/Qt-5/bin:$PATH gmake -j 6 PREFIX=/usr CC=gcc
cd ..
