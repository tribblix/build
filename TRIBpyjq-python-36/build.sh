...
#
env PATH=/usr/gnu/bin:$PATH /usr/versions/python-3.6/bin/python3 setup.py build
rm -fr /tmp/pil
env PATH=/usr/gnu/bin:$PATH /usr/versions/python-3.6/bin/python3 setup.py install --root=/tmp/pil
${THOME}/build/create_pkg TRIBpyjq-python-36 /tmp/pil
cd ..
rm -fr /tmp/pil
