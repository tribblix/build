...
cd ..
cd qt-everywhere-src-5.15.5-64bit
env PATH=/usr/gnu/bin:$PATH ./configure --prefix=/usr/versions/Qt-5 -opensource --disable-reduce-exports -no-sql-sqlite -xcb -xcb-xlib -bundled-xcb-xinput -confirm-license -nomake examples -nomake tests -c++std c++11 -bindir /usr/versions/Qt-5/bin/amd64 -libdir /usr/versions/Qt-5/lib/amd64 -plugindir /usr/versions/Qt-5/plugins/amd64 -importdir /usr/versions/Qt-5/imports/amd64 -qmldir /usr/versions/Qt-5/qml/amd64 -platform solaris-g++-64
env PATH=/usr/gnu/bin:$PATH gmake -j 8
cd ..
...
rm -fr /tmp/qt5
cd qt-everywhere-src-5.15.5
env PATH=/usr/gnu/bin:$PATH gmake -j 8 install INSTALL_ROOT=/tmp/qt5
cd ..
cd qt-everywhere-src-5.15.5-64bit
env PATH=/usr/gnu/bin:$PATH gmake -j 8 install INSTALL_ROOT=/tmp/qt5
cd ..
