...
env TRIBBLIX_CFLAGS=-Wno-incompatible-pointer-types ${THOME}/build/dobuild -64only zabbix-7.0.27 -P /opt/tribblix/zabbix -C "--enable-server --with-libcurl --with-postgresql=/opt/tribblix/postgres17/bin/pg_config --with-libpcre2 --with-libevent=/tmp/mc"
${THOME}/build/genpkg TRIBblix-zabbix zabbix-7.0.27
rm -fr /tmp/mc
