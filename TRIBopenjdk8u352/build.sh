...
# ga for 8u352 was b08
#
cd ~/ud
${THOME}/build/unpack openjdk8u352-b08
cd jdk8u352-b08
...
#
env PATH=${HOME}/sunstudio12.1/bin:/usr/bin:/usr/sbin bash ./configure --with-milestone=fcs --with-update-version=352 --with-build-number=b08 --enable-unlimited-crypto --with-boot-jdk=/usr/jdk/instances/jdk1.8.0 --with-cups=${THOME}/build/patches/jdk-headers --with-jobs=3
...
rm -fr /tmp/jdk
mkdir -p /tmp/jdk/usr/versions/openjdk1.8.0_352
mkdir -p /tmp/jdk/usr/jdk/instances
(cd build/solaris-x86_64-normal-server-release/images/j2sdk-image; tar cf - *) | ( cd /tmp/jdk/usr/versions/openjdk1.8.0_352 ; tar xf -)
cd /tmp/jdk/usr/jdk/instances
ln -s ../../versions/openjdk1.8.0_352 jdk1.8.0
cd /tmp/jdk/usr/jdk
ln -s ../versions/openjdk1.8.0_352 .
# latest is now jdk11
#ln -s openjdk1.8.0_352 latest
...
#rm `find . -name '*.diz'`
cp /tmp/cacerts /tmp/jdk/usr/versions/openjdk1.8.0_352/jre/lib/security
...
${THOME}/build/create_pkg TRIBopenjdk8u352 /tmp/jdk
