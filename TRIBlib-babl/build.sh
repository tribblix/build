...
env PKG_CONFIG_PATH=/usr/lib/amd64/pkgconfig meson --prefix=/usr -Denable-gir=false -Denable-mmx=false -Denable-sse=false --libdir=lib/amd64 -Dc_args=-m64 -Dc_link_args=-m64 build
cd build
ninja
cd ..
cd ..
