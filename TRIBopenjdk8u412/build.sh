...
# eg
# https://openjdk-sources.osci.io/openjdk8/openjdk8u412-ga.tar.xz
#
# check the build number corresponding to GA here
# https://github.com/openjdk/jdk8u/tags
# ga for 8u412 was b08
#
cd ~/ud
${THOME}/build/unpack jdk8u-jdk8u412-ga
...
--with-milestone=fcs --with-update-version=412 --with-build-number=b06 \
...
rm -fr /tmp/jdk
mkdir -p /tmp/jdk/usr/versions/openjdk1.8.0_412
mkdir -p /tmp/jdk/usr/jdk/instances
(cd build/solaris-x86_64-normal-server-release/images/j2sdk-image; tar cf - *) | ( cd /tmp/jdk/usr/versions/openjdk1.8.0_412 ; tar xf -)
cd /tmp/jdk/usr/jdk/instances
ln -s ../../versions/openjdk1.8.0_412 jdk1.8.0
cd /tmp/jdk/usr/jdk
ln -s ../versions/openjdk1.8.0_412 .
# latest is now jdk11
#ln -s openjdk1.8.0_412 latest
...
#rm `find . -name '*.diz'`
cp /tmp/cacerts /tmp/jdk/usr/versions/openjdk1.8.0_412/jre/lib/security
...
${THOME}/build/create_pkg TRIBopenjdk8u412 /tmp/jdk
