...
#
env TRIBBLIX_CFLAGS="-D__EXTENSIONS__ -D_LARGEFILE_SOURCE=64" PATH=/usr/gnu/bin:$PATH $THOME/build/mesonbuild -64only zathura-0.5.2
...
# now the plugins
#
env TRIBBLIX_CFLAGS="-D__EXTENSIONS__ -D_LARGEFILE_SOURCE=64" PATH=/usr/gnu/bin:$PATH $THOME/build/mesonbuild -64only zathura-pdf-poppler-0.3.1
