...
# eg
# https://openjdk-sources.osci.io/openjdk8/openjdk8u372-ga.tar.xz
#
# check the build number corresponding to GA here
# https://github.com/openjdk/jdk8u/tags
# ga for 8u372 was b07
#
cd ~/ud
${THOME}/build/unpack jdk8u-jdk8u372-ga
cd jdk8u-jdk8u372-ga
...
#
env PATH=${HOME}/sunstudio12.1/bin:/usr/bin:/usr/sbin bash ./configure --with-milestone=fcs --with-update-version=372 --with-build-number=b07 --enable-unlimited-crypto --with-boot-jdk=/usr/jdk/instances/jdk1.8.0 --with-cups=${THOME}/build/patches/jdk-headers --with-jobs=3
...
rm -fr /tmp/jdk
mkdir -p /tmp/jdk/usr/versions/openjdk1.8.0_372
mkdir -p /tmp/jdk/usr/jdk/instances
(cd build/solaris-x86_64-normal-server-release/images/j2sdk-image; tar cf - *) | ( cd /tmp/jdk/usr/versions/openjdk1.8.0_372 ; tar xf -)
cd /tmp/jdk/usr/jdk/instances
ln -s ../../versions/openjdk1.8.0_372 jdk1.8.0
cd /tmp/jdk/usr/jdk
ln -s ../versions/openjdk1.8.0_372 .
# latest is now jdk11
#ln -s openjdk1.8.0_372 latest
...
#rm `find . -name '*.diz'`
cp /tmp/cacerts /tmp/jdk/usr/versions/openjdk1.8.0_372/jre/lib/security
...
${THOME}/build/create_pkg TRIBopenjdk8u372 /tmp/jdk
