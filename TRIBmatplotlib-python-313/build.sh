...
#
env CXXFLAGS=-m64 CFLAGS=-m64 CC="gcc -m64" PATH=/usr/versions/python-3.13/bin:$PATH ${THOME}/build/pkg_pep518 -N -C "--config-settings=setup-args=-Dsystem-freetype=true --config-settings=setup-args=-Db_lto=false" TRIBmatplotlib-python-313 matplotlib-3.10.9
