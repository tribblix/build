...
#
env PKG_CONFIG_PATH=/usr/lib/`$THOME/build/getarch`/pkgconfig ./configure --prefix=/opt/tribblix/php --with-ldap=no --with-apxs2=/opt/tribblix/apache2/bin/apxs --enable-bcmath --enable-mbstring --enable-mysqlnd --with-pgsql=shared,/opt/tribblix/postgres17 --with-pdo-pgsql=shared,/opt/tribblix/postgres17 --with-pdo-mysql=mysqlnd --with-mysqli=mysqlnd --without-sqlite3 --without-pdo-sqlite --with-curl=/usr --enable-intl=shared --enable-gd --enable-exif --with-jpeg --with-bz2 --with-zlib --with-freetype --enable-sockets --enable-fpm --with-zip --with-openssl --with-sodium --with-gmp --enable-ftp --disable-mbregex --build=x86_64-pc-solaris2.11 CFLAGS="-m64 -D_XPG6 -D__EXTENSIONS__" LDFLAGS=-m64 ICU_CFLAGS=-m64

gmake -j 6

rm -fr /tmp/m4 /tmp/m4g /tmp/m4i /tmp/pear
mkdir -p /tmp/m4/opt/tribblix/apache2/conf
cp /opt/tribblix/apache2/conf/original/httpd.conf /tmp/m4/opt/tribblix/apache2/conf

gmake install INSTALL_ROOT=/tmp/m4

mkdir -p /tmp/m4g/opt/tribblix/php/lib/php/extensions/no-debug-zts-20240924
mkdir -p /tmp/m4i/opt/tribblix/php/lib/php/extensions/no-debug-zts-20240924
mv /tmp/m4/opt/tribblix/php/lib/php/extensions/no-debug-zts-20240924/*pgsql*.so /tmp/m4g/opt/tribblix/php/lib/php/extensions/no-debug-zts-20240924
mv /tmp/m4/opt/tribblix/php/lib/php/extensions/no-debug-zts-20240924/*intl*.so /tmp/m4i/opt/tribblix/php/lib/php/extensions/no-debug-zts-20240924

${THOME}/build/create_pkg TRIBblix-php-84 /tmp/m4
${THOME}/build/create_pkg TRIBblix-php-84pg /tmp/m4g
${THOME}/build/create_pkg TRIBblix-php-84intl /tmp/m4i
