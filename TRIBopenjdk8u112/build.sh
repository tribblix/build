...
wget http://hg.openjdk.java.net/jdk8u/jdk8u/${file}/archive/jdk8u112-b15.tar.bz2 -O ${file}-jdk8u112-b15.tar.bz2
end
pbzcat jdk8u112-b15.tar.bz2 | tar xf -
mv jdk8u-jdk8u112-b15 openjdk8u112
foreach file ( corba hotspot jaxp jaxws langtools jdk nashorn )
pbzcat ${file}-jdk8u112-b15.tar.bz2 | tar xf -
mv ${file}-jdk8u112-b15 openjdk8u112/$file
end
tar cf ${THOME}/tarballs/openjdk8u112.tar openjdk8u112
gzip ${THOME}/tarballs/openjdk8u112.tar

cd ~/ud
${THOME}/build/unpack openjdk8u112
cd openjdk8u112
...
#
env PATH=${HOME}/sunstudio12.1/bin:/usr/bin:/usr/sbin bash ./configure --with-milestone=fcs --with-update-version=112 --with-build-number=b15 --enable-unlimited-crypto
...
rm -fr /tmp/jdk
mkdir -p /tmp/jdk/usr/versions/openjdk1.8.0_112
mkdir -p /tmp/jdk/usr/jdk/instances
(cd build/solaris-x86_64-normal-server-release/images/j2sdk-image; tar cf - *) | ( cd /tmp/jdk/usr/versions/openjdk1.8.0_112 ; tar xf -)
cd /tmp/jdk/usr/jdk/instances
ln -s ../../versions/openjdk1.8.0_112 jdk1.8.0
cd /tmp/jdk/usr/jdk
ln -s ../versions/openjdk1.8.0_112 .
ln -s openjdk1.8.0_112 latest
...
rm `find . -name '*.diz'`
cp /tmp/cacerts /tmp/jdk/usr/versions/openjdk1.8.0_112/jre/lib/security
...
${THOME}/build/create_pkg TRIBopenjdk8u112 /tmp/jdk
