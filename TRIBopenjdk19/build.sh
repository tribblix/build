...
cd ~/ud
${THOME}/build/unpack jdk19-jdk-19-35
cd jdk19-jdk-19-35
...
# OpenJDK Runtime Environment (build 19-internal-adhoc.ptribble.jdk19-jdk-19-35)
# OpenJDK 64-Bit Server VM (build 19-internal-adhoc.ptribble.jdk19-jdk-19-35, mixed mode, sharing)
#

rm -fr /tmp/jdk
mkdir -p /tmp/jdk/usr/versions/openjdk19
mkdir -p /tmp/jdk/usr/jdk/instances
(cd build/solaris-x86_64-server-release/images/jdk; tar cf - *) | ( cd /tmp/jdk/usr/versions/openjdk19 ; tar xf -)
cd /tmp/jdk/usr/jdk/instances
ln -s ../../versions/openjdk19 jdk19
cd /tmp/jdk/usr/jdk
ln -s ../versions/openjdk19 .
#jdk19 isn't widely supported, so don't make it the default
#ln -s openjdk19 latest
...
#
# the jdk19 build appears to create a working one, but it uses
...
#
cp /tmp/cacerts /tmp/jdk/usr/versions/openjdk19/lib/security
...
(cd /tmp/jdk/usr/versions/openjdk19/conf/security/ ; gpatch -p1 < ${THOME}/build/patches/sunpkcs11-solaris.cfg.patch)

${THOME}/build/create_pkg TRIBopenjdk19 /tmp/jdk
